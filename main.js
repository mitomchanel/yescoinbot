...
class YesCoinBot {
	...
	async checkAndJoinBTC100K(token, proxy) {
		const listUrl = 'https://bi.yescoin.gold/activity/list/BTC100K';
		const checkInUrl = 'https://bi.yescoin.gold/activity/checkIn/BTC100K';

		try {
			const listResponse = await this.makeRequest('get', listUrl, null, token, proxy);
			if (listResponse.code === 0 && listResponse.data && listResponse.data.items) {
				const items = listResponse.data.items;
				const canJoinItem = items.find(item => item.status === 0 && item.allowJoin === 1);
				if (canJoinItem) {
					const checkInResponse = await this.makeRequest('get', checkInUrl, null, token, proxy);
					if (checkInResponse.code === 0) {
						await this.log('Checkin BTC100K thành công!', 'success');
					} else {
						await this.log(`Checkin BTC100K thất bại: ${checkInResponse.message}`, 'error');
					}
				} else {
					await this.log('Hôm nay đã checkin BTC100K.', 'info');
				}
			} else {
				await this.log(`Không thể lấy danh sách BTC100K: ${listResponse.message}`, 'error');
			}
		} catch (error) {
			await this.log(`Lỗi khi xử lý checkIn BTC100K: ${error.message}`, 'error');
		}
	}
    ...
    async performTasks() {
        try {
			...
			await this.checkAndJoinBTC100K(this.token, this.proxy);
        ...
        function startWorker() {
            if (accountQueue.length === 0) {
                if (activeWorkers === 0) {
                    ...
                    setTimeout(processCycle, 24 * 60 * 60 * 1000);
                }
                return;
            }
